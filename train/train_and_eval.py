...
def train_epoch(model, train_loader, loss_intent_shift,loss_intent,loss_slots, optimizer, p_u,data_source,is_lack):
    model.train()
    ...
    for d in tqdm.tqdm(train_loader):
        ...
        l = (
            loss_intent_shift(intent_shift_logits.flatten(), intent_shift_true_label.float().flatten())+
            # loss_intent(intent_logits.view((-1, intent_logits.shape[-1])), intent_true_label.long().flatten())+
            loss_slots(slots_logits.view((-1,slots_logits.shape[-1])), slots_true_label.long().flatten()))
        loss_sum += l.item()
        optimizer.zero_grad()
        l.backward()
        optimizer.step()
...
def eval(model, train_loader,loss_intent_shift, loss_intent, loss_slots, p_u, data_source, is_lack):
    model.eval()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    all_true_intent, all_pred_intent = [], []
    all_true_slots, all_pred_slots = [], []
...
def train(model, train_loader, test_loader, optimizer, epoch, loss_intent_shift,loss_intent,loss_slots,p_u,data_source,is_lack,file):
    for i in range(epoch):
        l_train, tis_train,pis_train,ti_train, pi_train,ts_train,ps_train = train_epoch(model, train_loader,loss_intent_shift,loss_intent,loss_slots, optimizer,p_u,data_source,is_lack)
        l_test, tis_test,pis_test,ti_test, pi_test,ts_test, ps_test = eval(model, test_loader, loss_intent_shift,loss_intent,loss_slots,p_u,data_source,is_lack)
        print("epoch:", i + 1)
        ...
        # utills.get_metrics_intent(ti_test, pi_test, False,file)
...
if __name__ == '__main__':
    ...
    for data_source in ["sim","atis","multiwoz"]:
        ...
        for method in ["ACM","AAAI","BERT"]:
            ...
            train_iter=DataLoader(dataset=MyDataset(tokenizer,max_len,data_source,True,is_lack),batch_size=32,shuffle=False)
            test_iter=DataLoader(dataset=MyDataset(tokenizer,max_len,data_source,False,is_lack),batch_size=32,shuffle=False)
            loss_intent_shift=torch.nn.BCEWithLogitsLoss()
            loss_intent=torch.nn.CrossEntropyLoss()
            loss_slots=torch.nn.CrossEntropyLoss()
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            model.to(device)
